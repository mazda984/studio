...
function updatePlayer(dt) {
  ...
  for (const o of solids) {
    ...
    // For other objects, test AABB overlap at the proposed newX.
    const overlapsHorizontally =
      o.type === "terrain"
        ? terrainIntersects(o, newX, player.y, player.w, player.h)
          ...
          for (let step = 1; step <= MAX_STEP_UP; step += 4) {
            ...
            const collisionAtStep = solids.some((other) => {
              if (other === o) {
                ...
                return newX < other.x + other.w && newX + player.w > other.x && testY < other.y + other.h && testY + player.h > other.y;
              } else {
                // For other solids, standard AABB test at the lifted position
                if (other.type === "terrain") {
                  return terrainIntersects(other, newX, testY, player.w, player.h);
                }
                return newX < other.x + other.w && newX + player.w > other.x && testY < other.y + other.h && testY + player.h > other.y;
