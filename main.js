...
// Info popup handler: shows the "Where do I share my games?" message with the requested URL.
...
function showInfoPopup() {
  ...
  popup.innerHTML = `
    <div class="title">Where do I share my games?</div>
    <div class="url">https://devdexentertainment.wixstudio.com/devdex</div>
...
function pointerDown(evt) {
  ...
  if (currentTool === "resize" || hit.part === "handle") {
...
function spawnPlayer() {
  // Find first spawn object, otherwise center of canvas
  ...
  if (spawn) {
    ...
    y = spawn.y - h; // appear above spawn block
  }
...
function updatePlayer(dt) {
  ...
  for (const o of solids) {
    ...
    const overlapsHorizontally =
      newX < o.x + o.w && newX + player.w > o.x;
    ...
    if (overlapsHorizontally && overlapsVertically) {
      ...
      if (o.type === "colliding" && o.dynamic) {
        ...
        // Nudge the block to avoid deep penetration and give immediate feedback
        if (player.vx > 0) {
        ...
        // Non-dynamic or regular solids block the player as before
        if (player.vx > 0) {
          resolvedX = o.x - player.w;
        } else if (player.vx < 0) {
          resolvedX = o.x + o.w;
        }
        player.vx = 0;
...
function updateProjectiles(dt) {
  ...
  if (projectiles.length > 0) {
    ...
    objects.forEach((o) => {
      ...
      projectiles.forEach((p) => {
        ...
        if (o.npcRespawn) {
          // Respawn NPC at its saved spawn position
          o.x = o.spawnX;
          o.y = o.spawnY;
          o.vx = 0;
          o.vy = 0;
          ...
          // We avoid removing it from the project so its editor placement persists.
          if (typeof o._savedX !== "number") o._savedX = o.x;
          if (typeof o._savedY !== "number") o._savedY = o.y;
          o.visible = false;
          o._hitDead = true;
  ...
  objects.forEach((o) => {
    ...
    // Prevent leaving left/right bounds (bounce a little)
    if (o.x < 0) {
      o.x = 0;
      o.vx = 0;
    }
    ...
    for (const other of solids) {
      // Treat all objects as solid except other dynamic colliding blocks for full symmetry (we still resolve them too).
      // perform AABB overlap test
      const overlapX = Math.max(0, Math.min(o.x + o.w, other.x + other.w) - Math.max(o.x, other.x));
      const overlapY = Math.max(0, Math.min(o.y + o.h, other.y + other.h) - Math.max(o.y, other.y));
      ...
      if (overlapX > 0 && overlapY > 0) {
        ...
        if (overlapX < overlapY) {
          ...
          if (o.x < other.x) {
            // push left side of other
            o.x -= overlapX;
            // damp horizontal velocity when hitting a wall/box
            if (o.vx > 0) o.vx *= 0.2;
          } else {
            o.x += overlapX;
            if (o.vx < 0) o.vx *= 0.2;
          }
          // clamp after resolution
          if (o.x < 0) o.x = 0;
          if (o.x + o.w > worldWidth) o.x = worldWidth - o.w;
        } else {
          // Separate vertically
          if (o.y < other.y) {
            // landed on top of other
            o.y -= overlapY;
            // land: zero vertical velocity and apply small restitution
            o.vy = Math.min(o.vy * -o.restitution, 0);
            // settle on top: tiny upward nudge avoidance handled by above
          } else {
            // hit from below
            o.y += overlapY;
            // hitting underside: small downward impulse
            if (o.vy < 0) o.vy = 0;
          }
          // clamp after resolution
          if (o.y + o.h > worldHeight) {
    ...
    // Very small velocity threshold to settle the object
...
function setPlayMode(on) {
  ...
  if (on) {
    ...
    // Restore colliding blocks to their saved editor positions so they don't remain where they were pushed.
    objects.forEach((o) => {
      if (o.type === "colliding" && typeof o._savedX === "number" && typeof o._savedY === "number") {
        o.x = o._savedX;
        o.y = o._savedY;
        // clear runtime velocities and saved markers
        o.vx = 0;
        o.vy = 0;
        delete o._savedX;
        delete o._savedY;
...
// Properties
[propName, propX, propY, propW, propH, propColor].forEach((input) => {
...
function encodeStateForUrl(state) {
  try {
    ...
    // Only persist to localStorage when CAN_PERSIST is true (prevents saving from embedded/foreign hosts).
    const MAX_INLINE_LENGTH = 15000; // tuned threshold for safety (approx characters)
    if (b64.length > MAX_INLINE_LENGTH) {
      if (!CAN_PERSIST) {
        // When persistence is not allowed, fall back to inline (may produce very long URLs but avoids cross-site writes).
        console.warn("Large payload detected but persistence disabled in this context; returning inline payload.");
        return b64;
      }
      try {
        const key = `ministudio_blob_${Date.now()}_${Math.floor(Math.random() * 10000)}`;
        // store the raw base64 JSON string so decode is simple
        localStorage.setItem(key, b64);
        // return a reference token the rest of the system understands (prefix "ref:")
        return `ref:${key}`;
      } catch (err) {
        // If storing fails, fall back to returning the inline version
        console.warn("Failed to store large state in localStorage, falling back to inline:", err);
        return b64;
      }
    }
