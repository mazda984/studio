...
function objectsTouching(a, b) {
  if (!a || !b) return false;
  // Only consider colliding blocks for this feature
  if (a.type !== "colliding" || b.type !== "colliding") return false;
...
function updateGlueHintForPointer(worldX, worldY) {
  ...
  // Find a cluster of touching colliding objects whose contact region is near the pointer.
  ...
  // Collect candidate colliding objects that are touching any other colliding object
  const candidates = [];
  for (let i = objects.length - 1; i >= 0; i--) {
    const a = objects[i];
    if (a.type !== "colliding") continue;
    for (let j = objects.length - 1; j >= 0; j--) {
      if (i === j) continue;
      const b = objects[j];
      if (b.type !== "colliding") continue;
